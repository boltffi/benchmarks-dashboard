...
def parse_args() -> argparse.Namespace:
    ...
    default_source_root = repository_root.parent / "mobiFFI" / "benchmarks"
    default_output_root = repository_root / "public" / "data"

    parser = argparse.ArgumentParser(
        description="Sync real benchmark data into the frontend public/data directory."
    )
    parser.add_argument(
        "--source-root",
        type=Path,
        default=default_source_root,
        help="Path to the mobiFFI benchmarks directory containing benchmark_run.json files.",
    )
    parser.add_argument(
        "--output-root",
        type=Path,
        default=default_output_root,
        help="Destination directory for generated public benchmark data.",
    )
...
def main() -> None:
    ...
    incoming_run_documents = load_incoming_run_documents(source_root)
    ...
    machine_entries = []
    for machine_id, machine_scoped_runs in machine_runs.items():
    ...
    for group_name, grouped_runs in group_runs.items():
        ...
        group_detail_path = output_root / "views" / "groups" / f"{slugify(group_name)}.json"
        json_dump(group_detail_document, group_detail_path)
    ...
    for benchmark_id, benchmark_runs in benchmark_runs_by_id.items():
        ...
        json_dump(benchmark_detail_document, benchmark_view_path)
    ...
    json_dump(benchmark_index_document, output_root / "catalog" / "benchmarks" / "index.json")
